...
import { OcupacionDetailsGeneralSection } from "@features/admin/modules/catalogos/ocupaciones/components/OcupacionDetailsGeneralSection";
...
import {
  ocupacionDetailsSchema,
  type OcupacionDetailsFormValues,
...
import { useOcupacionDetail } from "@features/admin/modules/catalogos/ocupaciones/queries/useOcupacionDetail";
import { getOcupacionErrorMessage } from "@features/admin/modules/catalogos/ocupaciones/utils/ocupaciones.feedback";
import {
  formatDate,
  formatDateTime,
} from "@features/admin/modules/catalogos/ocupaciones/utils/ocupaciones.format";
...
import type { OcupacionListItem, UpdateOcupacionRequest } from "@api/types";

interface OcupacionDetailsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onClose?: () => void;
  ocupacionSummary: OcupacionListItem | null;
  canEdit: boolean;
}

const DEFAULT_FORM_VALUES: OcupacionDetailsFormValues = {
  name: "",
};

const FORM_ID = "ocupacion-details-form";

export function OcupacionDetailsDialog({
  open,
  onOpenChange,
  onClose,
  ocupacionSummary,
  canEdit,
}: OcupacionDetailsDialogProps) {
  const { isClosing, markClosing, handleOpenChange } =
    useDetailsDialogCloseGuard(onOpenChange);

  const itemId = ocupacionSummary?.id;
  ...
  const {
    ...
    error: detailError,
    refetch,
  } = useOcupacionDetail(itemId, open && Boolean(itemId));

  const detail = detailResponse?.occupation;
  const updateOcupacion = useUpdateOcupacion();

  const form = useForm<OcupacionDetailsFormValues>({
    resolver: zodResolver(ocupacionDetailsSchema),
  ...
  const handleSave = async (values: OcupacionDetailsFormValues) => {
    ...
    try {
      await updateOcupacion.mutateAsync({ id: detail.id, data: payload });
      toast.success("Ocupacion actualizada", {
        description: "Los cambios se guardaron correctamente.",
      });
      form.reset(values);
    } catch (error) {
      toast.error("No se pudo guardar", {
        description: getOcupacionErrorMessage(error, "Error al guardar cambios"),
      });
    }
  };
  ...
  const handleStatusChange = async (nextActive: boolean) => {
    ...
    try {
      ...
      toast.success(nextActive ? "Ocupacion activada" : "Ocupacion desactivada");
    } catch (error) {
      toast.error("No se pudo actualizar el estado", {
        description: getOcupacionErrorMessage(error, "Error al actualizar estado"),
      });
    }
  };

  const title = detail?.name || ocupacionSummary?.name || "Ocupacion";
  const isActive = detail?.isActive ?? ocupacionSummary?.isActive;
  ...
  const errorContent = (
    <div className="rounded-2xl border border-line-struct bg-paper p-6 text-center">
      ...
      <h3 className="mt-4 text-base font-semibold text-txt-body">
        No se pudo cargar la ocupacion
      </h3>
    ...
    ? [
        {
          ...
          content: (
            <>
              <OcupacionDetailsGeneralSection
                form={form}
                formId={FORM_ID}
                ocupacionDetail={detail}
                onSubmit={handleSave}
                onStatusChange={handleStatusChange}
                isStatusPending={updateOcupacion.isPending}
                isEditable={canEdit}
              />
              {!canEdit ? (
                <AdminReadOnlyNotice message="Solo lectura: no puedes actualizar esta ocupacion porque no tienes permisos." />
              ) : null}
            </>
          ),
        },
  ...
  return (
    <AdminDetailsDialogShell
      ...
      titleSrOnly="Detalle de ocupacion"
      descriptionSrOnly="Gestiona la configuracion de esta ocupacion."
      header={
        ocupacionSummary || detail ? (
      ...
      footer={({ onCancel }) => (
        <CatalogDetailsFooter
          isDirty={isDirty}
          isSaving={updateOcupacion.isPending}
          formId={FORM_ID}
          onCancel={onCancel}
          disableSave={!canEdit}
        />
