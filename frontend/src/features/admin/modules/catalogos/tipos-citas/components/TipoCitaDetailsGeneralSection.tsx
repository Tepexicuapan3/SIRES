...
import type { TipoCitaDetail } from "@api/types";
import {
  CATALOG_STATUS,
  type CatalogStatus,
} from "@features/admin/modules/catalogos/shared/domain/catalog-status";
import type { TipoCitaDetailsFormValues } from "@features/admin/modules/catalogos/tipos-citas/domain/tipos-citas.schemas";

interface TipoCitaDetailsGeneralSectionProps {
  form: UseFormReturn<TipoCitaDetailsFormValues>;
  formId: string;
  tipoCitaDetail: TipoCitaDetail;
  onSubmit: (values: TipoCitaDetailsFormValues) => void;
  onStatusChange?: (nextActive: boolean) => void;
  isStatusPending?: boolean;
  isEditable?: boolean;
}

export function TipoCitaDetailsGeneralSection({
  form,
  formId,
  tipoCitaDetail,
  onSubmit,
  ...
  isEditable = true,
}: TipoCitaDetailsGeneralSectionProps) {
  const statusValue: CatalogStatus = tipoCitaDetail.isActive
    ...
    <Form {...form}>
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label>ID</Label>
            <Input value={tipoCitaDetail.id.toString()} disabled />
          </div>
        </div>
