...
import type { EdoCivilDetail } from "@api/types";
import {
  CATALOG_STATUS,
  type CatalogStatus,
} from "@features/admin/modules/catalogos/shared/domain/catalog-status";
import type { EdoCivilDetailsFormValues } from "@features/admin/modules/catalogos/edo-civil/domain/edoCivil.schemas";

interface EdoCivilDetailsGeneralSectionProps {
  form: UseFormReturn<EdoCivilDetailsFormValues>;
  formId: string;
  edoCivilDetail: EdoCivilDetail;
  onSubmit: (values: EdoCivilDetailsFormValues) => void;
  onStatusChange?: (nextActive: boolean) => void;
  isStatusPending?: boolean;
  isEditable?: boolean;
}

export function EdoCivilDetailsGeneralSection({
  form,
  formId,
  edoCivilDetail,
  onSubmit,
  ...
  isEditable = true,
}: EdoCivilDetailsGeneralSectionProps) {
  const statusValue: CatalogStatus = edoCivilDetail.isActive
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label>ID</Label>
            <Input value={edoCivilDetail.id.toString()} disabled />
          </div>
          <div className="space-y-2">
            <Label>Estado</Label>
            <Select
