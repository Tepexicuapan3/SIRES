...
import type { TurnoDetail } from "@api/types";
import {
  CATALOG_STATUS,
  type CatalogStatus,
} from "@features/admin/modules/catalogos/shared/domain/catalog-status";
import type { TurnoDetailsFormValues } from "@features/admin/modules/catalogos/turnos/domain/turnos.schemas";

interface TurnoDetailsGeneralSectionProps {
  form: UseFormReturn<TurnoDetailsFormValues>;
  formId: string;
  turnoDetail: TurnoDetail;
  onSubmit: (values: TurnoDetailsFormValues) => void;
  onStatusChange?: (nextActive: boolean) => void;
  isStatusPending?: boolean;
  isEditable?: boolean;
}

export function TurnoDetailsGeneralSection({
  form,
  formId,
  turnoDetail,
  onSubmit,
  ...
  isEditable = true,
}: TurnoDetailsGeneralSectionProps) {
  const statusValue: CatalogStatus = turnoDetail.isActive
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label>ID</Label>
            <Input value={turnoDetail.id.toString()} disabled />
          </div>
          <div className="space-y-2">
            <Label>Estado</Label>
            <Select
