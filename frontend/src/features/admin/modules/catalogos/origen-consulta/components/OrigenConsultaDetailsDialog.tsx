...
import { OrigenConsultaDetailsGeneralSection } from "@features/admin/modules/catalogos/origen-consulta/components/OrigenConsultaDetailsGeneralSection";
...
import {
  origenConsultaDetailsSchema,
  type OrigenConsultaDetailsFormValues,
...
import { useOrigenConsultaDetail } from "@features/admin/modules/catalogos/origen-consulta/queries/useOrigenConsultaDetail";
import { getOrigenConsultaErrorMessage } from "@features/admin/modules/catalogos/origen-consulta/utils/origenConsulta.feedback";
import {
  formatDate,
  formatDateTime,
} from "@features/admin/modules/catalogos/origen-consulta/utils/origenConsulta.format";
...
import type { OrigenConsultaListItem, UpdateOrigenConsultaRequest } from "@api/types";

interface OrigenConsultaDetailsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onClose?: () => void;
  origenConsultaSummary: OrigenConsultaListItem | null;
  canEdit: boolean;
}

const DEFAULT_FORM_VALUES: OrigenConsultaDetailsFormValues = {
  name: "",
};

const FORM_ID = "origen-consulta-details-form";

export function OrigenConsultaDetailsDialog({
  open,
  onOpenChange,
  onClose,
  origenConsultaSummary,
  canEdit,
}: OrigenConsultaDetailsDialogProps) {
  const { isClosing, markClosing, handleOpenChange } =
    useDetailsDialogCloseGuard(onOpenChange);

  const itemId = origenConsultaSummary?.id;
  ...
  const {
    ...
    error: detailError,
    refetch,
  } = useOrigenConsultaDetail(itemId, open && Boolean(itemId));

  const detail = detailResponse?.consultationOrigin;
  const updateOrigenConsulta = useUpdateOrigenConsulta();

  const form = useForm<OrigenConsultaDetailsFormValues>({
    resolver: zodResolver(origenConsultaDetailsSchema),
  ...
  const handleSave = async (values: OrigenConsultaDetailsFormValues) => {
    ...
    try {
      await updateOrigenConsulta.mutateAsync({ id: detail.id, data: payload });
      toast.success("Origen de consulta actualizado", {
        description: "Los cambios se guardaron correctamente.",
      });
      form.reset(values);
    } catch (error) {
      toast.error("No se pudo guardar", {
        description: getOrigenConsultaErrorMessage(error, "Error al guardar cambios"),
      });
    }
  };
  ...
  const handleStatusChange = async (nextActive: boolean) => {
    ...
    try {
      ...
      toast.success(nextActive ? "Origen de consulta activado" : "Origen de consulta desactivado");
    } catch (error) {
      toast.error("No se pudo actualizar el estado", {
        description: getOrigenConsultaErrorMessage(error, "Error al actualizar estado"),
      });
    }
  };

  const title = detail?.name || origenConsultaSummary?.name || "Origen de consulta";
  const isActive = detail?.isActive ?? origenConsultaSummary?.isActive;
  ...
  const errorContent = (
    <div className="rounded-2xl border border-line-struct bg-paper p-6 text-center">
      ...
      <h3 className="mt-4 text-base font-semibold text-txt-body">
        No se pudo cargar el origen de consulta
      </h3>
    ...
    ? [
        {
          ...
          content: (
            <>
              <OrigenConsultaDetailsGeneralSection
                form={form}
                formId={FORM_ID}
                origenConsultaDetail={detail}
                onSubmit={handleSave}
                onStatusChange={handleStatusChange}
                isStatusPending={updateOrigenConsulta.isPending}
                isEditable={canEdit}
              />
              {!canEdit ? (
                <AdminReadOnlyNotice message="Solo lectura: no puedes actualizar este origen de consulta porque no tienes permisos." />
              ) : null}
            </>
          ),
        },
  ...
  return (
    <AdminDetailsDialogShell
      ...
      titleSrOnly="Detalle de origen de consulta"
      descriptionSrOnly="Gestiona la configuracion de este origen de consulta."
      header={
        origenConsultaSummary || detail ? (
      ...
      footer={({ onCancel }) => (
        <CatalogDetailsFooter
          isDirty={isDirty}
          isSaving={updateOrigenConsulta.isPending}
          formId={FORM_ID}
          onCancel={onCancel}
          disableSave={!canEdit}
        />
