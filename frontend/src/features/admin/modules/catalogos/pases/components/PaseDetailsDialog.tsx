...
import { PaseDetailsGeneralSection } from "@features/admin/modules/catalogos/pases/components/PaseDetailsGeneralSection";
...
import { usePaseDetail } from "@features/admin/modules/catalogos/pases/queries/usePaseDetail";
import { getPaseErrorMessage } from "@features/admin/modules/catalogos/pases/utils/pases.feedback";
import {
  formatDate,
  formatDateTime,
} from "@features/admin/modules/catalogos/pases/utils/pases.format";
...
import type { PaseListItem, UpdatePaseRequest } from "@api/types";

interface PaseDetailsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onClose?: () => void;
  paseSummary: PaseListItem | null;
  canEdit: boolean;
}

const DEFAULT_FORM_VALUES: PaseDetailsFormValues = {
  name: "",
};

const FORM_ID = "pase-details-form";

export function PaseDetailsDialog({
  open,
  onOpenChange,
  onClose,
  paseSummary,
  canEdit,
}: PaseDetailsDialogProps) {
  const { isClosing, markClosing, handleOpenChange } =
    useDetailsDialogCloseGuard(onOpenChange);

  const itemId = paseSummary?.id;
  ...
  const {
    ...
    error: detailError,
    refetch,
  } = usePaseDetail(itemId, open && Boolean(itemId));

  const detail = detailResponse?.pass;
  const updatePase = useUpdatePase();

  const form = useForm<PaseDetailsFormValues>({
    resolver: zodResolver(paseDetailsSchema),
  ...
  const handleSave = async (values: PaseDetailsFormValues) => {
    ...
    try {
      await updatePase.mutateAsync({ id: detail.id, data: payload });
      ...
      toast.error("No se pudo guardar", {
        description: getPaseErrorMessage(error, "Error al guardar cambios"),
      });
    }
  };
  ...
  const handleStatusChange = async (nextActive: boolean) => {
    ...
    try {
      ...
      toast.success(nextActive ? "Pase activado" : "Pase desactivado");
    } catch (error) {
      toast.error("No se pudo actualizar el estado", {
        description: getPaseErrorMessage(error, "Error al actualizar estado"),
      });
    }
  };

  const title = detail?.name || paseSummary?.name || "Pase";
  const isActive = detail?.isActive ?? paseSummary?.isActive;
    ...
    ? [
        {
          ...
          content: (
            <>
              <PaseDetailsGeneralSection
                form={form}
                formId={FORM_ID}
                paseDetail={detail}
                onSubmit={handleSave}
                onStatusChange={handleStatusChange}
                isStatusPending={updatePase.isPending}
                isEditable={canEdit}
              />
              {!canEdit ? (
                <AdminReadOnlyNotice message="Solo lectura: no puedes actualizar este pase porque no tienes permisos." />
              ) : null}
            </>
          ),
        },
  ...
  return (
    <AdminDetailsDialogShell
      ...
      titleSrOnly="Detalle de pase"
      descriptionSrOnly="Gestiona la configuracion de este tipo de pase."
      header={
        paseSummary || detail ? (
          <PaseDialogHeader
            title={title}
