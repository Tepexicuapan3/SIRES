...
import { CheckCircle2, Loader2, QrCode, RotateCcw, ScanLine } from "lucide-react";
    ...
    <section className="space-y-5 p-6">
      <header className="space-y-2 rounded-xl border border-line-struct bg-paper p-5">
        <div className="inline-flex items-center gap-2 rounded-full border border-line-hairline bg-subtle px-3 py-1 text-xs font-medium text-txt-muted">
          ...
          Recepción · Check-in por QR
        </div>
        <h1 className="text-2xl font-semibold text-txt-body">
          Check-in por código QR
        </h1>
        <p className="max-w-2xl text-sm text-txt-muted">
          Escanea el comprobante de la cita con el lector de códigos, o pega
          el código manualmente si el lector falla, y presiona Enter para
          validarlo.
        </p>
      </header>
      ...
      <Card className="max-w-xl">
        <CardHeader>
          ...
          <CardDescription>
            El campo se mantiene enfocado para escaneos consecutivos, uno por
            paciente.
          </CardDescription>
        </CardHeader>
      ...
      {ficha ? (
        <Card className="max-w-xl">
          <CardHeader>
            <CardTitle>Confirmar datos del paciente</CardTitle>
            <CardDescription>
              Verifica que la persona frente a ti coincida con estos datos
          ...
          <CardContent className="space-y-4">
            ...
            <dl className="grid gap-3 text-sm sm:grid-cols-2">
              ...
              <div>
                ...
                <dd className="font-medium text-txt-body">
                  {formatFechaHora(ficha.fechaHora)}
                </dd>
              </div>
              <div>
                <dt className="text-[10px] font-semibold uppercase tracking-wide text-txt-muted">
