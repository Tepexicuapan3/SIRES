...
} from "@/domains/auth-access/components/admin/rbac/roles/RolesTableColumns";
import {
...
import { getRoleErrorMessage } from "@/domains/auth-access/adapters/rbac/roles/roles.feedback";
...
export function RolesPage() {
  ...
  const canCreateRole = resolveCapability("admin.roles.create", {
    allOf: ["admin:gestion:roles:create"],
  });
  const canUpdateRole = resolveCapability("admin.roles.update", {
    allOf: ["admin:gestion:roles:update"],
  });
  const canDeleteRole = resolveCapability("admin.roles.delete", {
    allOf: ["admin:gestion:roles:delete"],
  });
  const canReadRole = resolveCapability("admin.roles.read", {
  ...
  const canEditDialog = resolveCapability("admin.roles.editFull", {
    allOf: [
  ...
  const tableErrorDescription = error
    ...
    : undefined;
  const capabilityErrorDescription = RBAC_CAPABILITIES_DEGRADED_MESSAGE.roles;
  ...
  const tableOptions: TableOptionItem[] = [
    {
      ...
      disabled: isFetching || !canReadRole,
      onSelect: () => {
        if (isFetching || !canReadRole) {
          return;
        }
